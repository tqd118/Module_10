...
export default function ProfileInfo() {
    ...
    const validationSchema = object().shape({
        firstName: string().required(t("forms.requiredField", { field: t("profile.firstName") })),
        secondName: string().required(t("forms.requiredField", { field: t("profile.secondName") })),
        username: string()
            .required(t("forms.requiredField", { field: t("profile.username") }))
            .matches(/^[a-zA-Z0-9_]+$/, t("profile.usernameNotValid")),
        email: string()
            .required(t("forms.requiredField", { field: t("auth.email") }))
    ...
    const { profileImage, firstName, secondName, description, email, username } = formik.values;
    const { handleChange, errors } = formik;
    const [fullNameInput, setFullNameInput] = useState(`${firstName} ${secondName}`.trim());

    return (
        <div className={s.info}>
            <form className={s.profile} id="changeProfile" onSubmit={formik.handleSubmit}>
                <h2 className={s.title}>{t("profile.editProfile")}</h2>

                <div className={s.user}>
                    <Image 
                        src={getAssetUrl(profileImage) || getAssetUrl("/assets/blank-user.png")} 
                        alt="avatar"
                        width={64}
                        height={64}
                    />
                ...
                <label className={s.inputLabel}>
                    ...
                    />
                    {(formik.touched.firstName || formik.touched.secondName) && (errors.firstName || errors.secondName) && (
                        <span className={s.error}>{errors.firstName || errors.secondName}</span>
                    )}
                </label>
                ...
                <label className={s.inputLabel}>
                    ...
                    <span className={s.descriptionWarn}>
                        {errors.description ? (
                            <span className={`icon-info ${s.error}`}>{t("profile.reachedCharsLimit")}</span>
                        ) : (
                ...
                <Button className={s.saveInfoButton} type="submit" form="changeProfile">
                    {t("profile.saveChanges")}
                </Button>
            </form>

            <form className={s.preferences} onSubmit={handleLogout}>
